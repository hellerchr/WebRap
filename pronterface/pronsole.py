...
class pronsole(cmd.Cmd):
    ...
    def help_load(self):
        print "Loads a store file (with tab-completion)"
    ...
    def help_upload(self):
        print "Uploads a store file to the sd card"
    
    
    def help_print(self):
        if self.f is None:
            print "Send a loaded store file to the printer. Load a file with the load command first."
        else:
            print "Send a loaded store file to the printer. You have "+self.filename+" loaded right now."
    ...
    def do_skein(self,l):
        ...
        try:
            ...
            if(settings):
                ...
                self.do_load(l[0].replace(".stl","_export.store"))
        except Exception,e:
    ...
    def help_skein(self):
        print "Creates a store file from an stl model using skeinforge (with tab-completion)"
        print "skein filename.stl - create store file"
        print "skein filename.stl view - create store file and view using skeiniso"
